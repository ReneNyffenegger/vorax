...
function! voraxlib#omni#Meets(context)
  if len(a:context) >= 2
    return 1
  else
  	return 0
...
function! voraxlib#omni#Complete(findstart, base)
  ...
  if a:findstart
    ...
    let result = ['dba_tables', 'dba_users', 'dba_views']
    return result
...
function! s:ComputeCompletionContext()
  ...
  if s:context.head =~ '[(,]\_s*[a-zA-Z0-9_#$]*$'
    ...
    let s:context.complete_from = match(s:context.head, '\(\((\|,\)\_s*\)\@<=\([0-9a-zA-z#$_]*$\)')
    let s:context.prefix = strpart(s:context.head, s:context.complete_from)
  elseif s:context.head =~ '\.[0-9a-zA-Z#$_]*$'
    let s:context.type = 'dot'
    let s:context.complete_from = match(s:context.head, '\(\.\)\@<=\([0-9a-zA-z#$_]*$\)')
    let s:context.prefix = matchstr(s:context.head, '\(\s*\)\@<=\([0-9a-zA-z#$_.]*$\)')
  elseif s:context.head =~ '\s*[0-9a-zA-Z#$_]*$'
    let s:context.type = 'word'
    let s:context.complete_from = match(s:context.head, '\(\s*\)\@<=\([0-9a-zA-z#$_]*$\)')
    let s:context.prefix = strpart(s:context.head, s:context.complete_from)
  endif
endfunction
