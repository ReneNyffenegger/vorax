...
setlocal indentkeys+==~end,=~else,=~elseif,=~elsif,=~when,0=~exception,0=)
...
function s:GetStmtStarterIndent( keyword, curr_lnum )
    ...
    elseif a:keyword =~? 'when'
        ...
        let matching_lnum = searchpair(
                    ...
                    \ 'bW',
                    \ 's:IsColComment(line("."), col(".")) == 1')
        echom string(matching_lnum) . ':' . string(a:curr_lnum)
        echom string(s:ModuloIndent(indent(matching_lnum)))
        exec 'normal! $'
        if matching_lnum > 0 && matching_lnum < a:curr_lnum
            let ind = indent(matching_lnum) + &shiftwidth
        else
