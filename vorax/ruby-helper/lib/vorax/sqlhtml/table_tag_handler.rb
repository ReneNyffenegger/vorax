module Vorax
  ...
  class TableTagHandler < AbstractTagHandler
    ...
    def vertical(node)
      buffer = ''
      columns = []
      node.xpath("tr[1]/th").each { |col| columns << CGI.unescapeHTML(col.text).strip }
      ...
      node.xpath('tr/*').each do |tr|
        if tr.name == 'td'
          value = CGI.unescapeHTML(tr.text.mb_chars.gsub(/\n/, ' ')).strip
          buffer << columns[idx] << ': ' << value << "\n"
          idx += 1
    ...
    def compressed(node)
      ...
      (0..cols-1).each do |rn|
        structure << node.xpath("tr/th[#{rn+1}]|tr/td[#{rn+1}]").inject([0, false]) do |result, element| 
          [ [result[0], CGI.unescapeHTML(element.text.mb_chars).mb_chars.gsub(/\r?\n/, ' ').strip.length].max, (result[1] || element.attribute('align'))] 
        end
      end
      line = []
      idx = 0
      ...
      node.xpath('tr/*').each do |tr|
        ...
        value = CGI.unescapeHTML(tr.text.mb_chars.gsub(/\r?\n/, ' ').strip)
        if structure[idx][1]
